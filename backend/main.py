...
def update_grade(
    ...
    grade.subject_name = grade_data.subject_name
    db.commit()
    db.refresh(grade)
...
def get_student_qr_code(
    ...
    # In a real app, you might generate and store the QR code
    # For now, we'll return the profile URL that can be encoded as QR
    base_url = "http://localhost:3000"  # Configure this based on your frontend URL
    profile_url = f"{base_url}/dashboard/students/{student_code}"
